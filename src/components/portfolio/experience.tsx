...
export function Experience() {
  return (
    <Section id="experience" label="Experience" title="Professional journey & milestones.">
      <div className="relative border-l border-border pl-6 sm:pl-10 ml-4 space-y-8">
        {ROLES.map((r, i) => {
          const Icon = r.icon;
          return (
            <Reveal key={r.role} delay={i * 90} className="relative">
              {/* Glowing Node Marker */}
              <span className="absolute -left-[calc(1.5rem+13px)] sm:-left-[calc(2.5rem+13px)] top-1 flex h-6 w-6 items-center justify-center rounded-full bg-card border border-primary text-primary shadow-sm">
                <Icon size={12} />
              </span>
              ...
              <div className="bento-card p-6 sm:p-8">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="rounded-full bg-foreground/5 px-3 py-1 font-mono text-xs font-semibold text-muted-foreground">
                    {r.company}
                  </span>
                </div>

                <h3 className="mt-3 text-xl font-bold tracking-tight text-foreground">{r.role}</h3>

                <ul className="mt-4 space-y-2.5">
                  {r.points.map((p) => (
                    <li
                      key={p}
                      className="relative pl-5 text-xs sm:text-sm leading-relaxed text-muted-foreground before:absolute before:left-0 before:top-[0.6em] before:h-1.5 before:w-1.5 before:rounded-full before:bg-primary/70"
                    >
                      {p}
                    </li>
                  ))}
